...
# This an R file with which r repetitions (number of meta-analysis) can be generated and the PI limits can be calculated for them with the HTS methods and the Ensemble method.

# r (the number of independent repetitions) and all other parameters (RE distribution, K, N, etc.) can be set in the Base_parameters.R file.

# m, the number of "runs" can be specified, this defines how many times this code will be run, each with r repetitions.

# A basedir folder should be set, where the output files will be saved in separate folders.

# In the basedir folder, an "RE_distributions" folder should be created beforehand, in which folders with the names of the distributions should be created manually. 

# The folders in the "RE_distributions" folder should be named "Normal", "Bimodal", "Uniform", "Skewed_Normal_50", "Skewed_Normal_75", "Skewed_Normal_99". 
...
# Set basedir folder, in which an "RE_distributions" folder should be created beforehand, in which folders with the names of the distributions should be created beforehand, manually.  

basedir <- paste0(getwd(),"/")
  
#basedir <- "D:/Matrai_Peter/PI_Article/Sim_To_Article_Revision_2/"
      ...
      for (g in 1:length(n_pattern)) {        # n of total patients
        for (h in 1:length(k)) {              # n of studies
          for (i in 1:r) {                    # which simulation
            ...
            res_list[["HTS_HK_k_1_PI_lower"]][i , h, g, t]  <- Temp_list$HTS_HK_k_1_PI[1]
            res_list[["HTS_HK_k_1_PI_upper"]][i , h, g, t]  <- Temp_list$HTS_HK_k_1_PI[2]
