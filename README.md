...
4. Generating input data
    ...
    * In the **Sim_Cont_Many.R** file you can set how many times to run the generating process. This was necessary for us, because when we had not enough memory to generate 5000 repetitions. Instead, we run the whole process 5 times with 1000 repetitions and merged them in a later step. We call these **Runs**. You can set the number of runs in line 26 as defining a for loop. The default is `for (m in 1:1) {`, which means that the process will be run one time. You only need to set more runs if you set **r** to a high number and you run out of memory.  
