...
knit: (function(inputFile, encoding) { 
    rmarkdown::render(
        ...
        output_file = file.path(
            dirname(inputFile), paste0("Sim_PI", ".html"))) 
    })
---
...
# In this next chunk you can set:

#         - a basedir, the location of the "Base_parameters.R"  file (by default, this is the folder where the "PI_Sim_Matrai.Rproj" file is located)
#         - which distribution and which runs you want to display
#         - which methods you want to display
#         - if you want to display the parametric bootstrap method (Nag_to_display)
...
# As it is explained in the Github Readme file:

# The files (randomly generated MAs from different distributions) loaded, processed and visualized in this .Rmd are 
#                                         -  either should be downloaded from "https://mega.nz/folder/d7o0VI6I#IDIanHGoP21XiwcZEPaYhg"  
#                                         -  or you can set up a folder system manually in your basedir folder and generate MAs yourself and use those
...
# you can set the runs in the form 1:n, it means that runs from 1 to n will be processed and displayed. 

#There are 5 runs from each distribution (each containing 1000 repetitions) used in the simulation of the article.


Normal_to_display <- 1
Normal_run_to_display <- 1:5

Uniform_to_display <- 0
...
HTS_DL_to_display <- 1
HTS_REML_to_display <- 0    # by default, this method is set to 0 because it gives nearly identical results to the HTS-DL (t, K-2) method
HTS_HK_REML_to_display <- 1
HTS_DL_k_1_to_display <- 1 
HTS_HK_k_1_to_display <- 1
...
# you can load and display this method only if you download the necessary files from https://mega.nz/folder/d7o0VI6I#IDIanHGoP21XiwcZEPaYhg. 
# For example, for the normal distrbution it is in the folder called "Normal" with the name "Res_list_Nag_Normal_Run_1.RData". 
# You can find the files for the other distributions similarly. 

Nag_to_display <- 1

# you can set the runs for the the parametric bootstrap method in the form 1:n, it means that runs from 1 to n will be processed and displayed. There is only 1 run available for this method with 1000 repetitions in the simulation of the article.
...
# Set TRUE if you want to display the histograms of coverage probabilities (Hist_to_display) or set FALSE if you do not want. 

Hist_to_display <-        T

# Set TRUE if you want to display the other 5 plots (Other_plots_to_display) or set FALSE if you do not want.
...
<!--- No need to set anything from here --->
```{r preparations}
...
all_colours <- c("blue4", "purple4", "olivedrab", "turquoise2", "deeppink3", "mediumpurple1",  "sienna")
...
if(Nag_to_display == 1) {pch_to_display <- c(pch_to_display, 7 ) }
 ...
 for (i in 1:length(method_names)) {  # loop over the methods
  ...
  CDF_function <- CDF_normal
    ...
    for (i in 1:length(t2)) {
      ...
      HTS_HK_k_1_quantile_cov_Normal[, , ,i] <-  CDF_function(HTS_HK_k_1_PI_upper_Normal[, , ,i], mean = mu, sd=sqrt(t2[i]) ) - CDF_function(HTS_HK_k_1_PI_lower_Normal[, , ,i], mean = mu, sd=sqrt(t2[i]) )
  ...
  CDF_function <- CDF_uniform
    ...
    for (i in 1:length(t2)) {
      ...
      HTS_DL_k_1_quantile_cov_Uniform[, , ,i] <-  CDF_function(HTS_DL_k_1_PI_upper_Uniform[, , ,i], mean = mu, sd=sqrt(t2[i]) ) - CDF_function(HTS_DL_k_1_PI_lower_Uniform[, , ,i], mean = mu, sd=sqrt(t2[i]) )
       ...
       HTS_HK_k_1_quantile_cov_Uniform[, , ,i] <-  CDF_function(HTS_HK_k_1_PI_upper_Uniform[, , ,i], mean = mu, sd=sqrt(t2[i]) ) - CDF_function(HTS_HK_k_1_PI_lower_Uniform[, , ,i], mean = mu, sd=sqrt(t2[i]) )
 ...
 for (i in 1:length(method_names)) {  # loop over the methods
  ...
  CDF_function <- CDF_skewed_normal_50
    ...
    for (i in 1:length(t2)) {
     ...
     HTS_HK_k_1_quantile_cov_Skewed_Normal_50[, , ,i] <-  CDF_function(HTS_HK_k_1_PI_upper_Skewed_Normal_50[, , ,i], mean = mu, sd=sqrt(t2[i]) ) - CDF_function(HTS_HK_k_1_PI_lower_Skewed_Normal_50[, , ,i], mean = mu, sd=sqrt(t2[i]) )
 ...
 for (i in 1:length(method_names)) {  # loop over the methods
  ...
  CDF_function <- CDF_skewed_normal_75
    ...
    for (i in 1:length(t2)) {
     ...
     HTS_HK_k_1_quantile_cov_Skewed_Normal_75[, , ,i] <-  CDF_function(HTS_HK_k_1_PI_upper_Skewed_Normal_75[, , ,i], mean = mu, sd=sqrt(t2[i]) ) - CDF_function(HTS_HK_k_1_PI_lower_Skewed_Normal_75[, , ,i], mean = mu, sd=sqrt(t2[i]) )
 ...
 for (i in 1:length(method_names)) {  # loop over the methods
  ...
  CDF_function <- CDF_skewed_normal_99
    ...
    for (i in 1:length(t2)) {
      ...
      HTS_HK_k_1_quantile_cov_Skewed_Normal_99[, , ,i] <-  CDF_function(HTS_HK_k_1_PI_upper_Skewed_Normal_99[, , ,i], mean = mu, sd=sqrt(t2[i]) ) - CDF_function(HTS_HK_k_1_PI_lower_Skewed_Normal_99[, , ,i], mean = mu, sd=sqrt(t2[i]) )
  ...
  CDF_function <- CDF_bimodal
    ...
    for (i in 1:length(t2)) {
      ...
      HTS_DL_k_1_quantile_cov_Bimodal[, , ,i] <-  CDF_function(HTS_DL_k_1_PI_upper_Bimodal[, , ,i], mean = mu, sd=sqrt(t2[i]) ) - CDF_function(HTS_DL_k_1_PI_lower_Bimodal[, , ,i], mean = mu, sd=sqrt(t2[i]) )
        ...
        HTS_HK_k_1_quantile_cov_Bimodal[, , ,i] <-  CDF_function(HTS_HK_k_1_PI_upper_Bimodal[, , ,i], mean = mu, sd=sqrt(t2[i]) ) - CDF_function(HTS_HK_k_1_PI_lower_Bimodal[, , ,i], mean = mu, sd=sqrt(t2[i]) )
...
Observed_per_true_length_plot <- function(n, tau2, distribution,
 ...
 ", mean I2 = ",paste0(round(get(paste0("mean_I2_",distribution)) [length(k), n, tau2],0), "%") ),
          cex.main=0.9,
          xlab = "Number of studies",
          ylab = "Mean observed length / theoretical length",
          xtics = k,
          ylim = ylim,
          yticlab=NA,
          ytics = F ,
...
# Define quantile function to draw vertical line on plots
  ...
  legend("topleft", "Theoretical PI boundaries defined as 2.5% and 97.5% percentiles \n of the true effects distribution", lty=1, col="lightgreen",
...
# **Mean observed length relative to the theoretical length** {.tabset .tabset-pills}
