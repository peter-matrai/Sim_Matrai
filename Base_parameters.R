...
#--------------------------------------------------------------------------------
# This is an R file containing packages to load, base settings and functions.
...
library("dplyr")      # we used:version 1.1.3
library("readxl")     # we used:version 1.4.3
library("metafor")    # we used:version 4.8-0
library("meta")       # we used:version 6.0-0
library("pimeta")     # we used:version 1.1.3
library("plotrix")    # we used:version 3.8-2
library("abind")      # we used:version 1.4-5
library("sn")         # we used:version 2.1.1       # for skewed normal distribution
library("moments")    # we used:version 0.14.1      # for skewed normal distribution
library("stringr")    # we used:version 1.5.0       # for str_sub() function
library("EnvStats")   # we used:version 2.8.1       # for the bimodal distribution which is a mixture of 2 normals

#-----------------------------------------------------------------------------------------------------------

# Setting the distribution. Only relevant for Sim_Cont_Many.R. 
...
r <- 1000         # number of meta-analysis to simulate
                  # you can set it to a smaller number if the data generation takes too long time

#-----------------------------------------------------------------------------------------------------------

# Other default simulation parameter settings used in the article

mu<- 0            # mean of true effects
...
# Set random effects distribution based on distribution set above
...
varlist_results <- c(  "true_PI_lower", "true_PI_upper", "teta_new",
                       ...
                       "HTS_HK_k_1_PI_lower", "HTS_HK_k_1_PI_upper",
...
PI_function <- function(y_i, v_i, level = PI_level) {
  ...
  res$HTS_HK_k_1_PI <- c( res$mean_REML - res$t_k_1*sqrt(res$tau2_REML + res$SE_HK_REML^2) , res$mean_REML + res$t_k_1*sqrt(res$tau2_REML + res$SE_HK_REML^2 ) )
